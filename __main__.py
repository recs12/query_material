...
def create_various_queries(asm, search_subassemblies):
    ...
    create_query(asm.Queries, "Material [STEEL PLATE]", [steel_plate.criterias], search_subassemblies)

    # "SS.PLATE"
    ss_plate = CriteriaProperties(
        seQueryPropertyCustom, "JDEDSC1_A", seQueryConditionContains, "SS.PLATE"
    )
    create_query(asm.Queries, "Material [SS.PLATE]", [ss_plate.criterias], search_subassemblies)

    # "ALU"
    alu_plate = CriteriaProperties(
        seQueryPropertyCustom, "JDEDSC1_A", seQueryConditionContains, "AL.PLATE"
    )
    create_query(asm.Queries, "Material [AL.PLATE]", [alu_plate.criterias], search_subassemblies)
...
def user_confirmation_to_continue():
    response = raw_input(
        """Would you like to create material queries in the Select Tools? (Press y/[Y] to proceed.)"""
    )
